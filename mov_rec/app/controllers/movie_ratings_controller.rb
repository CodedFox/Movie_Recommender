class MovieRatingsController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @movie_rating.update(movie_rating_params)
        format.html { redirect_to '/users/', notice: 'Movie rating was successfully updated.' }
  ...
  private
    ...
    def movie_rating_params
      params.require(:movie_rating).permit(:date_watched, :user_rating, :review, :user_id, :movie_id)
    end
end
